        ...
        <div class="card border-primary ">
            ...
            <table class="table table-responsive-lg ">
                ...
                <tr v-for="rule in filteredRows" :value="rule.id">
                    ...
                    <td>
                        <button type="submit" class="btn btn-primary" @click="loadRules(rule.id)"> Show child rules
                        </button>
                    </td>
                    <td>
...
export default {
    ...
    mounted() {
        this.
        axios.get("http://localhost/laravelproject/public/api/parentRules").then(response => {
    ...
    methods: {
        loadRules($id) {
            axios.get("http://localhost/laravelproject/public/api/childRules/" + $id).then(response => {
                this.childRules = response.data;
                if (this.childRules.length !== 0) {
                    this.$router.push({name: 'childrules', params: {id: $id}})
                } else {
